...
class BlurSuite extends FunSuite {
  test("getSurrounding getting correct surrounding pixels") {
    val src = new Img(5, 5)

    for (x <- 0 until 5; y <- 0 until 5)
      src(x, y) = rgba(x, y, x + y, math.abs(x - y))

    assert(getSurrounding(src, 2, 2, 1).size == 9)
    assert(getSurrounding(src, 1, 1, 1).size == 9)
    assert(getSurrounding(src, 0, 0, 1).size == 4)
  }
